...
const NewTab = () => {
  ...
  return (
    <div className={`min-h-screen h-full flex justify-center xl:items-center items-start max-xl:py-12 w-screen min-w-[580px] overflow-auto NewTab`}>
      <div className="xl:grid xl:grid-cols-4 xl:h-[80vh] h-full min-h-[10rem] gap-x-3 xl:w-[1200px] max-xl:max-w-[700px] w-full max-xl:flex max-xl:flex-col-reverse max-xl:justify-center max-xl:gap-y-6 max-xl:items-center">
          <div className="w-full rounded-2xl bg-white gap-y-2 relative p-2 shadow-2xl xl:hidden">
            <CalenderWidget />
          </div>
        <TodoListWidget />
        <div className='col-span-2 max-xl:w-full '>
          <div className={`max-xl:flex max-xl:flex-col max-xl:items-center max-xl:max-w-[700px] ${!bookmarkFullVisible && "h-[250px] overflow-y-hidden"}`}>
          <SearchBar />
          <Shortcuts />
          <CodeEditorWidget />
          </div>
          <div className='flex justify-center items-center w-full mt-5'><div onClick={() => setBookmarkFullVisible(!bookmarkFullVisible)} className='text-[#666666] font-semibold flex justify-center items-end gap-x-1 cursor-pointer hover:opacity-50 transition-all duration-200'>{bookmarkFullVisible ? "Close" : "See More"} <img src={ChevronIcon} alt="Chevron" className={`w-5 h-5 ${bookmarkFullVisible && "rotate-180 transition-all duration-200"}`} /></div></div>
        </div>
        <div className="col-span-1 xl:flex xl:flex-col max-xl:grid max-xl:grid-cols-2 max-xl:gap-x-3 gap-y-3">
          ...
          <div className="w-full rounded-2xl bg-white flex flex-col justify-start items-center gap-y-2 relative p-2 shadow-2xl max-xl:hidden">
            <CalenderWidget />
          </div>
        </div>
      </div>
